...
We give the characterisation in terms of the following signature:

\begin{definition} A \textbf{single-sorted (finitary) word signature}, $\Psi$,
has $n \in \mathbb{N}$ symbols, $m \in \mathbb{N}$ of which, where $m \leq n$,
are nullary, and $\p{n-m}$ of which are unary. More formally, let

$$\Psi \triangleq \p{\set{-}, \set{v_0 : 0, \; \ldots, \; v_m : 0, \; v_{m+1} :
1, \; \ldots, \; v_n : 1}}.$$

\end{definition}

\begin{example} Let $B \triangleq \p{\set{-}, \set{\varepsilon : 0, \symb{0} :
1, \symb{1} : 1}}$ be a single-sorted word signature with the context-free
grammar
...
etc. are all terms over $B$. We will use $B$ in subsequent examples.
...
% that an empty string has a non-zero size is intuitively justifiable as even the
% lack of data is data in itself.
...
signature $\Psi$, with $i$ nullary and $\p{j-i}$ unary symbols, is a binary
symbol \smashf{}, written in infix notation, with the semantics given by
$\sem{x \smashf{} y} = i + \p{j-i}^{\card{x}\cdot\card{y}}$.\end{definition}

\begin{example} The smash function over $B$ is given by $\sem{x \smashf{} y} =
2^{\card{x} \cdot \card{y}}$. \end{example}

\begin{definition} A single-sorted word signature $\Sigma = \set{ \mathbf{B},
\mathbf{V} }$ ``enriched'' with a smash function is $\Sigma^\smashf{} = \set{
\mathbf{B}, \mathbf{V} \cup \set { \smashf{} : 2 } }$. \end{definition}

The semantics of bounded primitive recursion on notation is given by

$$\sem{\mathtt{BPRN}\p{g_0, \ldots, g_i, g_{i+1}, \ldots, g_j, k}} = f$$
...
f\p{c_0,y} &= \sem{g_0}\p{y} \\
          & \vdots \\
f\p{c_i,y} &= \sem{g_i}\p{y} \\
f\p{c_{i+1}\p{x},y} &= \sem{g_{i+1}}\p{x,y,f\p{x,y}} \\
          & \vdots \\
f\p{c_j\p{x},y} &= \sem{g_j}\p{x,y,f\p{x,y}} \\
...
\begin{theorem} The algebra $\algebra{\Sigma}{\mathtt{BPRN}}$ over a
single-sorted word signature $\Sigma$ is \FPTIME{}-sound.\end{theorem}
...
\begin{proof} Proof by simulation of an \FPTIME{} TM. \end{proof}
