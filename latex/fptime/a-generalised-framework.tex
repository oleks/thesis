...
It is of principle interest to mathematicians et al. to categorise functions
into classes and hierarchies. Some classes of functions are defined
\emph{extrinsically}, e.g.  the functions computable by means of a Turing
machine, while others are defined \emph{intrinsically}, e.g. by means of a
function algebra.

A \textbf{function algebra}, $\mathcal{F}$, is a superset of a chosen set of
\textbf{initial functions} $\mathcal{F}_I$, closed under a chosen set of
operations, $\mathcal{F}_O$. An \textbf{operation} is a function, which takes
functions in $\mathcal{F}$ as input. A class $\mathcal{F}$ is \textbf{closed
under} an operation, if the operation also yields a function in $\mathcal{F}$
as output.

For instance, one classical function algebra, which we'll call $\mathcal{P}$,
is a function algebra containing a zero and successor function as initial
functions, and is closed under the operation of composition.
...
between an intrinsic and extrinsic definitions of a class, which is prevalent
throughout this document. For our intents and purposes, every function algebra
is a class, but not every function class is an algebra. \end{remark} 

A function in $\mathcal{C}$ admits an interpretation as a \textbf{construction
tree}. A leaf is a particular initial function. An internal node is a
particular operation. The outgoing edges (of an internal node) model the
application of the operation to particular functions in $\mathcal{C}$,
interpreted as the respective, outbound subtrees.
...
$\mathtt{EXTR}$ is a generalised substitution operator. Using
$\mathtt{EXTR}$, we can obtain not only $\mathtt{PROJ}$ and
$\mathtt{COMP}$, but also identity and permutation functions.  On the
